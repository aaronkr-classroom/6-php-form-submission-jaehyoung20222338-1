...
if($_SERVER['REQUEST_METHOD'] == 'POST') { // 폼 제출 확인
  ...
  $user['terms'] = (isset($_POST['terms']) and $_POST['terms'] == true) ? true : false; // 동의서 확인

  $errors['name'] = is_text($user['name'], 2, 20) ? '': '2~20문자로 사용자 이름을 만드세요';
  $errors['age'] = is_number($user['age'], 16, 65) ? '' : '16~65살 사이를 입력하세요';
  $errors['terms'] = $user['terms'] ? '' : '동의해야 합니다!';

  $invalid = implode($errors);            // 오류 메세지
  if ($invalid){
    $message = '오류 해결하세요!!';
  } else {
    $message = '폼 제출해서 감사합니다!';
  }
}
